...
def process_escape_secuences(ch):
    if ch == '0':
       ch = '\0'
    elif ch == 'a':
       ch = '\a'
...
def process_single_constant(var_info, index):
    ...
    if var_info[NUM].token == 'infty':
       ...
       reg = '0'
    elif var_info[NUM].token == 'character':
       if var_info[NUM].value[1] == '\\':
          reg = process_escape_secuences(var_info[NUM].value[2])
       elif var_info[NUM].value[1] == ' ':
          reg = SPACE
       else:
          reg = bin(ord(var_info[NUM].value[1]))[2:]
    else:
       if index:
